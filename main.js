...
Game.registerMod("cookiegardenhelperreloaded", {
    ...
    build: function () {
        ...
        this.doc.elId('row2').insertAdjacentHTML('beforeend', `
            ...
            this.config.autoHarvestMatured
                    ...
                    <p>
					  ${this.button(
            'cpsRequireAllMature', 'Require All mature',
            'When harvest mature seed base on cps, check all seeds are mature/dying/new/empty', true,
            this.config.cpsRequireAllMature
        )}
					</p>
                    <p>
					  ${this.button(
            'freezeAllMature', 'Freeze All mature',
            'When all plants are mature freeze garden, unfreeze when harvested', true,
            this.config.freezeAllMature
        )}
					</p>
					<p>
					  ${this.button(
            ...
            this.config.autoHarvestMiniCpSMultDying
				...
				<div class="cookieGardenHelperReloadedPanel" id="autoPlantPanel">
                  ...
                  <p>
					${this.button(
            'autoPlantAllEmpty', 'All Empty',
            'Make sure there all slots that would be planted is empty', true,
            this.config.autoPlantAllEmpty
        )}
				  </p>
				  <p>
					${this.button(
    ...
    setCorrectSoil: function () {
        ...
        if (!M.freeze) {
            ...
            var targetSoil = young * 2 > matur ? soilCombo[1] : soilCombo[2];
    ...
    all_tiles_valid: function () {
        let all_tiles_valid_flag = true;

        this.forEachTile((x, y) => {
            // Empty tiles are always valid
            if (this.tileIsEmpty(x, y)) {
                return;
            }

            // Get the tile object and its plant
            const tile_object = this.getTile(x, y);
            const plant_object = this.getPlant(tile_object.seedId);

            // New seed (locked plant) is valid
            if (!plant_object.unlocked) {
                return;
            }

            // Mature plant is valid
            const plant_stage = this.getPlantStage(tile_object);
            if (plant_stage === 'mature') {
                return;
            }

            // dying plant is valid
            if (plant_stage === 'dying') {
                return;
            }

            // Anything else (young, dying, etc.) makes the whole garden invalid
            all_tiles_valid_flag = false;
        });

        return all_tiles_valid_flag;
    },
    all_intended_tiles_empty: function () {
        let all_intended_tiles_empty_flag = true;

        this.forEachTile((x, y) => {
            if (this.config.savedPlot[y][x][0] > 0 && (!this.tileIsEmpty(x, y))) { // If intend to plant something in this slot and the slot is not empty
                all_intended_tiles_empty_flag = false;
                return;
            }
        });

        return all_intended_tiles_empty_flag;
    },
    freezeAllMature: function () {
        var hasAllMatureCpsPlant = true;
        var allNonCps = true;
        this.forEachTile((x, y) => {
                if (!this.tileIsEmpty(x, y)) {
                    let tile = this.getTile(x, y);
                    if (!this.getPlant(tile.seedId).immortal && this.isCpsBonus(this.getPlant(tile.seedId))){
                        allNonCps = false;
                        let stage = this.getPlantStage(tile);
                        if (stage == 'young') {
                            hasAllMatureCpsPlant = false;
                        }
                    }
                }
            });
        if (this.minigame().freeze == 0){ // if not in freeze
            if (!allNonCps && hasAllMatureCpsPlant) { // if has cps plant and all cps plants are mature
                this.minigame().freeze = 1; // freeze it
            }
        }
        else { // if in freeze
            if (!hasAllMatureCpsPlant || allNonCps) { // and not all cps plants are mature or all plants are noncps/empty
                this.minigame().freeze = 0; // unfreeze it
            }
        }
    },
    handleMature: function (plant, x, y) {
        const all_tiles_valid_flag = this.all_tiles_valid() || !this.config.cpsRequireAllMature;

        if (!plant.unlocked && this.config.autoHarvestNewSeeds) {
            this.harvest(x, y);
        }
        else if (
            all_tiles_valid_flag &&
            this.isCpsBonus(plant) &&
            this.config.autoHarvestCheckCpSMult &&
            this.CpSMult() >= this.config.autoHarvestMiniCpSMult.value
        ) {
            this.harvest(x, y);
        }
        else if (
    ...
    run: function () {
        if (this.isActive()) {
            ...
            // freeze garden when all mature
            if (this.config.freezeAllMature) {
                this.freezeAllMature();
            }

            // check if all indended tiles are empty
            const is_all_intended_tiles_empty = !this.config.savedPlot.length > 0 || this.all_intended_tiles_empty();
            ...
            this.forEachTile((x, y) => {
                ...
                if (this.config.autoPlant &&
                    ...
                    (!this.config.autoPlantAllEmpty || is_all_intended_tiles_empty) &&
    ...
    defaultConfig: function () {
        var data = {
            ...
            cpsRequireAllMature: true,
            freezeAllMature: false,
